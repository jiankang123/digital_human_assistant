...
def plot_result(wav_path, pred_result, output_dir=None):
    ...
    print(output_dir)
    if not output_dir:
        pic_path = 'fig_batch_dir/fig_{}.png'.format(os.path.basename(wav_path))
    else:
...
def wav_test(wav_path, net, mean, scale, output_dir=None, plot=True, multi_out = False):
    fbank = compute_fbank(wav_path)
    fbank = (fbank - mean) * scale
    if multi_out:
        pred,_ = net(fbank.unsqueeze(0))
