...
from dataloader import hw_dataset,trans_test_Collate_fn
...
def train(args):
    ...
    # 数据准备
    train_dataset = hw_dataset(args.train_data, hotwords, logging=True)
    val_dataset = hw_dataset(args.val_data, hotwords, logging=True)
    ...
    for epoch in range(epochs):
        ...
        net = net.train()
        for batch, (X, l, y) in enumerate(train_dataloader):    # X.shape: (1, bs, seq_len, feat_dim)
            # X, l, y = X.squeeze(0), l.squeeze(0), y.squeeze(0)  # X.shape: (bs, seq_len, feat_dim)
            X = compute_fbank_matrix(X,use_gpu=use_gpu)
            l = (l / 160).long()
            if use_gpu:
            ...
            # compute CMVN
            X = (X - mean) * scale
            raw_y, y = y, F.one_hot(y.long(), num_classes=num_classes).float()   
            if use_mixup:
        ...
        with torch.no_grad():
            for X, l, y in val_dataloader:
                ...
                batch_correct = (pred_t.argmax(1) == raw_y).type(torch.float).sum().item()
