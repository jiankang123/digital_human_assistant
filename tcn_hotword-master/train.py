...
from configs import net_configs
...
class my_loss(nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        # test
        #labels *= torch.tensor([1.0, 0.9], device=labels.device)
...
def save_pt(net, epoch, batch, val_correct, val_loss, val_FRR, val_FAR, 
    ...
    top_corrects.append([epoch, batch, 
                         val_correct*100, val_loss, val_FRR*100, val_FAR*100, 
                         test_pos_correct*100, test_pos_loss, test_pos_FRR*100, test_pos_FAR*100,
                         test_neg_correct*100, test_neg_loss, test_neg_FRR*100, test_neg_FAR*100,
                         ])
    ...
    with open(os.path.join(output_dir, 'info.txt'), 'w', encoding='utf-8') as fout:
        ...
        for k in top_corrects:
            fout.write('\t'.join(["%.4f" % s for s in k]) + '\n')
...
def train(args):
    ...
    if pretrained_model:
        ...
        net = build_model(net_configs[net_config])
    if use_gpu:
    ...
    for epoch in range(epochs):
        ...
        batch_save_interval = train_size
        net = net.train()
        for batch, train_batch_data in enumerate(train_dataloader):    # X.shape: (1, bs, seq_len, feat_dim)
            ...
            # 前20个epoch限制唤醒触发时间，后面放开限制
            if args.has_boundary and epoch < 50:
...
def main():
    ...
    parser.add_argument('--net_config', help='net config', required=True)
    ...
    parser.add_argument('--avg', action="store_true", default=False, help='for downsample')
