...
def main():
    ...
    # save cpp_src
    cpp_src_filename = "weights_vivo_cmds_{}_20k_only_int8.h".format(
        date
    )
    ...
    # def test_quant():
    #     fbank = torch.load("test_fbank.pt")[0]
    #     mean, scale = get_CMVN("cmvn.json")
    #     fbank = (fbank - mean) * scale
    #     feat = fbank[0].float()
    #     qfeat = []
    #     for feati in feat:
    #         qfeat.append(float_to_q15(feati, 9))
    #     qfeat = torch.tensor(np.array(qfeat, dtype=np.int32))
    #     print(qfeat)
    #     w = torch.tensor(weights["layer0_w"]["int8_val"], dtype=torch.int32)
    #     print(w)
    #     x = torch.matmul(w, qfeat.reshape(-1, 1))
    #     print(x)
    #     # fx = q31_weight_to_float(x, weights["layer0_w"]["int8_frac"]+9)
    #     fx = x / np.power(2.0, 16)
    #     print(fx)
    #     float_result = torch.matmul(weights["layer0_w"]["float_val"], feat.reshape(-1,1))
    #     print(float_result)
    # test_quant()
